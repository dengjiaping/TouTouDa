						...
						postRunnable(new Runnable() {
							...
							public void run() {
								if (jsonResponse instanceof JSONObject) {
									LogUtil.d(((JSONObject) jsonResponse)
											.toString());
									...
									onFailure();
									// onFailure(statusCode, headers, (String)
									// jsonResponse, new
									// JSONException("Response cannot be parsed as JSON data"));
								} else {
									onFailure();
									// onFailure(statusCode, headers, new
					...
					try {
						...
						postRunnable(new Runnable() {
							...
							public void run() {
								 ...
								 * (JSONObject) jsonResponse); } else if
								 * (jsonResponse instanceof JSONArray) {
								 * onFailure(statusCode, headers, throwable,
								 * (JSONArray) jsonResponse); } else if
								 * (jsonResponse instanceof String) {
								 * onFailure(statusCode, headers, (String)
								 * jsonResponse, throwable); } else {
								 * onFailure(statusCode, headers, new
								 * JSONException("Unexpected response type " +
								 * jsonResponse.getClass().getName()),
								 * (JSONObject) null); }
