...
public class ProductListActivity extends BaseActivity implements OnRefreshListener,IProductGridListener{
	...
	void initView(){
		try {
			...
			if(cb!=null){
				list=new Gson().fromJson(cb.getValue(), new TypeToken<ArrayList<ProductListBean>>(){}.getType());
			}
			...
			public void onSuccess(JSONObject response) {
				try {
					...
					case 0:
						...
						cacheDao.create(cb);
						List<ProductListBean> list=new Gson().fromJson(response.getString(URL.RESPONSE), new TypeToken<ArrayList<ProductListBean>>(){}.getType());
